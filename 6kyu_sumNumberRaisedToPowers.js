...
// 90, 100 --> []
 function sumRaisedToPowers (a, b) {
    let output = []
    for (let i = a; i <= b; i++) {
        x = String(i).split('').reduce((acc, val, init) => acc + Math.pow(val, init + 1),0)
        if (x === i) {
            output.push(i)
        }
    } 
 ...
 console.log(sumRaisedToPowers(1,10))
 console.log(sumRaisedToPowers(1,100))
 console.log(sumRaisedToPowers(90,100))